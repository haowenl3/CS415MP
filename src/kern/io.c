...
// io_lit operation declarations
static long iolit_read(struct io_intf *io, void *buf, unsigned long bufsz);
static long iolit_write(struct io_intf *io, const void *buf, unsigned long n);
static void iolit_close(struct io_intf *io);
static int iolit_ctl(struct io_intf *io, int cmd, void *arg);
static const struct io_ops io_lit_ops;
...
{
    lit->io_intf.ops = &io_lit_ops;
    lit->buf = buf;
    lit->size = size;
    lit->pos = 0; // Start at the beginning of the buffer
    //           Implement me!
    return &lit->io_intf;
}
...
// io_lit read operation
static long iolit_read(struct io_intf *io, void *buf, unsigned long bufsz) {
    struct io_lit *lit = (struct io_lit *)((char *)io - offsetof(struct io_lit, io_intf));
    size_t available = lit->size - lit->pos;
...
// io_lit close operation
static void iolit_close(struct io_intf *io) {
    // No resources to release for a memory buffer.
}

// io_lit control operation
static int iolit_ctl(struct io_intf *io, int cmd, void *arg) {
    struct io_lit *lit = (struct io_lit *)((char *)io - offsetof(struct io_lit, io_intf));

    switch (cmd) {
        case IOCTL_SETPOS:
            if (*(size_t *)arg > lit->size)
                return -EINVAL;
            lit->pos = *(size_t *)arg;
            return 0;
        case IOCTL_GETPOS:
